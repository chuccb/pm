...
> 更新基準：2026-09-17。
> 文件責任：集中 `680–696` login/account-adjacent protocol 與 `197–221` MyInfo／ClientData 玩家資料同步。Character／Inventory 的 runtime/domain semantic 由 [`Character_Inventory_Equipment.md`](Character_Inventory_Equipment.md) 維護。
...
`n100` 後續會被 `143 PM_UDPSTART_REQ` 使用，因此存在跨 packet dependency。[C]
...
Client raw storage stride = 132 bytes；另有 80-byte `CLobbyServerData` view。不能用 132/80-byte object offset 當 wire offset。[C]
...
`sub_401B50()` 是 Client state update，不是第三個 wire string。[C]
...
## 6. Tutorial / application-control
...
`689` 是表面型別與實際 wire width 不一致的典型：實際 writer = 4 bytes。[C]

`694` negotiated limit 上限 `0x2580 = 9600`，之後轉入 `682`。[C]
...
不能直接猜成 public item enum。[C][OPEN]
...
後續依 `ack_mode`／descriptor family 分支；nested grammar 尚未完全閉合。[C][OPEN]
...
實際事件先後仍以 Client object/data-flow 為準，不靠 opcode 編號猜測。[C][OPEN]
...
大小：3 / 9 / 35 / 41 bytes，最多 4 records；主要用於 `203/220/221`。[C]
...
是 selective/delta synchronization，不是 Family A。[C]
...
`resource_id` 直接進 resource validation；invalid item 走 error 6。[C]
...
wire 順序不可依 C++ object offset、UI 顯示順序或推測 business order 重排。[C]
...
但 `MYINFO/AVATAR` 路徑不能單獨反推每一個 wire field 名稱。[C][RES][OPEN]
...
`200` 成功後初始化 item collection state。[C]

Client runtime 的 5120-entry inventory 與 wire Family-C 不可混為同一 struct；domain semantic 由 [`Character_Inventory_Equipment.md`](Character_Inventory_Equipment.md) 維護。
...
`201` 插入 collection；`202` 更新 collection，但部分 u32 semantics `[OPEN]`。[C][OPEN]
...
部分 tail semantics `[OPEN]`。[C]

### 208

雖表面 prototype 顯示 char，實際 writer 為 4-byte：
...
flag 後依條件最多讀 3 × u32；用途為 collection record 定位／移除候選，business semantics `[OPEN]`。[C]
...
`218` = Family-B delta；`220/221` = repeated Family-A。[C]

完整 public semantics 仍維持 raw／`[OPEN]`。
...
這是 runtime state，不代表每個 economy wire field 都已知道名稱。[C]
...
```text
681 server-list public fields
682 verification composite / hardware-ID exact semantics
695/696 descriptor-class grammar
